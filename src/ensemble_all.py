...
logger = logging.getLogger("ensemble_all")
...
def main():
    ...
    args.model1_type = "bert"
    args.model2_type = "xlnet"
    args.model3_type = "roberta"
    bert_config = BertConfig.from_pretrained(bert_type, num_labels=num_labels)
    xlnet_config = XLNetConfig.from_pretrained(
        xlnet_type, num_labels=num_labels)
    roberta_config = RobertaConfig.from_pretrained(
        roberta_type, num_labels=num_labels)

    bert_tokenizer = BertTokenizer.from_pretrained(bert_type)
    xlnet_tokenizer = XLNetTokenizer.from_pretrained(xlnet_type)
    ...
    if args.ensemble_pretrained_model:
        ...
        bert_model = BertGetPoolFeatures.from_pretrained(
            bert_type, config=bert_config)
        xlnet_model = XLNetGetPoolFeature.from_pretrained(
            xlnet_type, config=xlnet_config)
        roberta_model = RobertaGetPoolFeature.from_pretrained(
            roberta_type, config=roberta_config)
        # init ensemble model
        model = EnsembleAll(args, bert_config, xlnet_config, roberta_config,
                            bert_model, xlnet_model, roberta_model, num_labels)
    ...
    # Training
    if args.do_train:
        bert_dataset = load_and_cache_examples(
            args, "bert", args.task_name, bert_tokenizer, evaluate=False)
        xlnet_dataset = load_and_cache_examples(
            args, "xlnet", args.task_name, xlnet_tokenizer, evaluate=False)
        roberta_dataset = load_and_cache_examples(
            args, "roberta", args.task_name, roberta_tokenizer, evaluate=False)

        train_dataset = TensorDataset(
            *bert_dataset, *xlnet_dataset, *roberta_dataset)
    ...
    if args.do_eval and args.do_train:
        eval_dataset1 = load_and_cache_examples(
            args, "bert", args.task_name, bert_tokenizer, evaluate=True)
        eval_dataset2 = load_and_cache_examples(
            args, "xlnet", args.task_name, xlnet_tokenizer, evaluate=True)
        eval_dataset3 = load_and_cache_examples(
            args, "roberta", args.task_name, roberta_tokenizer, evaluate=True)
        eval_dataset = TensorDataset(
            *eval_dataset1, *eval_dataset2, *eval_dataset3)
    ...
    if args.do_pred:
        pred_dataset1 = load_and_cache_examples(
            args, "bert", args.task_name, bert_tokenizer, predict=True)
        pred_dataset2 = load_and_cache_examples(
            args, "xlnet", args.task_name, xlnet_tokenizer, predict=True)
        pred_dataset3 = load_and_cache_examples(
            args, "roberta", args.task_name, roberta_tokenizer, predict=True)
        pred_dataset = TensorDataset(
            *pred_dataset1, *pred_dataset2, *pred_dataset3)
        # model = torch.load(args.output_dir + "/pytorch_model.bin")
        model.to(args.device)
