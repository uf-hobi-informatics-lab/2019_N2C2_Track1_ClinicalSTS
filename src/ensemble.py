...
def main():
    ...
    parser.add_argument(
      ...
      default=None,
      type=str,
      help="Third model's type that you want to ensemble, choose from: " + 
      ", ".join(MODEL_CLASSES.keys()),
    )
    parser.add_argument(
      "--model3_name_or_path",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to pretrained ensemble model",
    )
    parser.add_argument(
        "--config1_name", 
        default="", 
        type=str, 
        help="Pretrained config name or path if not the same as model1_name",
    )
    parser.add_argument(
        "--tokenizer1_name",
        default="",
        type=str,
        help="Pretrained tokenizer name or path if not the same as model1_name",
    )
    parser.add_argument(
        "--config2_name", 
        default="", 
        type=str, 
        help="Pretrained config name or path if not the same as model2_name",
    )
    parser.add_argument(
        "--tokenizer2_name",
        default="",
        type=str,
        help="Pretrained tokenizer name or path if not the same as model2_name",
    )
    parser.add_argument(
        "--config3_name",
        default="",
        type=str,
        help="Pretrained config name or path if not the same as model3_name",
    )
    parser.add_argument(
        "--tokenizer3_name",
        default="",
        type=str,
        help="Pretrained tokenizer name or path if not the same as model3_name"
    )
    parser.add_argument(
        "--cache_dir",
        default="",
    ...
    if args.do_train:
        ...
        train_dataset3 = load_and_cache_examples(
            args, args.model3_type, args.task_name, tokenizer3, evaluate=False) if args.model3_type else None

        if train_dataset3:
            train_dataset = TensorDataset(*train_dataset1, *train_dataset2, *train_dataset3)
        else:
    ...
    if args.do_eval and args.do_train:
        ...
        eval_dataset3 = load_and_cache_examples(
            args, args.model3_type, args.task_name, tokenizer3, evaluate=True) if args.model3_type else None

        if eval_dataset3:
            eval_dataset = TensorDataset(*eval_dataset1, *eval_dataset2, *eval_dataset3)
        else:
